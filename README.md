...
[![Functions](https://img.shields.io/badge/Built--in%20Functions-65%2B-orange.svg)](docs/API_REFERENCE.md)
...
| ðŸ“‹ **Dynamic Lists** | Built-in arrays with 15+ operations (sort, slice, search, stats) |
| ðŸ”¤ **Rich String Library** | 14+ string functions â€” split, join, format, replace, trim, search |
| ðŸ§® **Math & Science** | Trig, logarithms, GCD/LCM, statistics, unit conversion |
| ðŸ“ **File I/O** | Read, write, append files â€” build real applications |
| ðŸŽ¯ **65+ Built-in Functions** | Covers strings, math, lists, files, types, system  |
...
|---|---|
| `prakar(x)` | Returns type name |
| `sankhya(x)` / `dashmlav(x)` / `purn(x)` | Convert to number/float/integer |
| `shabd(x)` | Convert to string |
| `kya_sankhya(x)` / `kya_shabd(x)` / `kya_suchi(x)` | Type checking |
| `kya_kaam(x)` / `kya_bool(x)` / `kya_khali(x)` / `kya_purn(x)` | Type checking |
...
| `sqrt_val(x)` / `power_val(b, e)` | Square root / Power |
| `sin_val` / `cos_val` / `tan_val` | Trigonometry |
| `asin_val` / `acos_val` / `atan_val` | Inverse trig |
| `log_val(x)` / `log10_val(x)` / `exp_val(x)` | Logarithms |
| `PI()` / `E()` | Math constants |
...
| `taareekh()` / `waqt()` / `timestamp()` | Date / Time |
...
â”‚   â”œâ”€â”€ native.h/c       â€” Built-in functions (65+)
...
â”œâ”€â”€ examples/            â€” Example .ram programs (15+)
â”œâ”€â”€ docs/
â”‚   â”œâ”€â”€ HOWTO.md         â€” How to Use guide
â”‚   â”œâ”€â”€ TUTORIAL.md      â€” Step-by-step tutorial
â”‚   â”œâ”€â”€ API_REFERENCE.md â€” All 65+ built-in functions
...
| **[API Reference](docs/API_REFERENCE.md)** | All 65+ built-in functions documented |
| **[Language Spec](docs/LANGUAGE_SPEC.md)** | Formal grammar & syntax rules |
| **[Examples](examples/)** | 15+ ready-to-run .ram programs |
